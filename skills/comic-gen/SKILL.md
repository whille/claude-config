...
| 参数 | 选项 | 说明 |
|------|------|------|
| **帧数密度** | 简洁(10-15帧) / 标准(15-25帧) / 详细(25-40帧) | 控制漫画长度 |
| **风格选择** | 水墨风格 / 写实风格 / 动漫风格 | 画面艺术风格 |
| **角色一致性** | 是 / 否 | 是否使用参考图绑定角色形象 |

#### 帧数密度说明

| 密度 | 帧数范围 | 适用场景 |
|------|----------|----------|
| 简洁 | 10-15 帧 | 快速预览、短篇章节 |
| 标准 | 15-25 帧 | 常规章节、完整叙事 |
| 详细 | 25-40 帧 | 重要章节、详细刻画 |

#### 帧数分配规则

| 情节类型 | 简洁 | 标准 | 详细 |
|----------|------|------|------|
| 开场 | 1-2 帧 | 2-3 帧 | 3-4 帧 |
| 对话 | 1-2 帧 | 2-3 帧 | 3-5 帧 |
| 动作 | 2-3 帧 | 3-5 帧 | 5-8 帧 |
| 转场 | 0-1 帧 | 1 帧 | 1-2 帧 |
| 收尾 | 1-2 帧 | 2-3 帧 | 3-4 帧 |

#### 风格说明

| 风格 | Prompt 关键词 | 适用作品 |
|------|----------------|----------|
| 水墨风格 | 水墨画风格，中国传统艺术，淡雅色调 | 古典小说 |
| 写实风格 | 写实风格，油画质感，精细细节，真实光影 | 现代小说 |
| 动漫风格 | 动漫风格，鲜艳色彩，清新明亮 | 轻小说 |

#### 角色一致性说明

- **是**：先生成首图作为参考，后续帧使用参考图绑定角色形象
- **否**：每帧独立生成，角色形象可能有变化
...
| 问题 | 正确做法 |
|------|----------|
| 对白臆造 | ❌ **绝对禁止臆造对白**，必须引用原文，原文无对白则留空 |
| 场景臆造 | ❌ **场景必须基于原文**，不能添加原文未提及的元素 |
| 总结联想 | ❌ **不能总结或联想**，每帧必须对应原文具体段落 |
| 时代错误 | ❌ **服饰/建筑必须符合时代**，严格遵循时代背景 |
| 文化错位 | ❌ **不能出现不符文化的元素**（如西方场景出现佛像） |
| 对白简化 | ❌ 必须引用原文对白，不能改写 |
